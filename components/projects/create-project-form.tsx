...
import { type ArtworkSearchResult, searchArtworks } from "@/lib/api/artworks";
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Create a project and pick 1 to 10 places from Art Institute results.
				</CardDescription>
			</CardHeader>
			<CardContent>
							...
							<Popover>
								...
								<PopoverContent className="w-auto p-0" align="start">
									<Calendar
										...
										onSelect={(date) => {
											startDateController.field.onChange(
						...
						<Field data-invalid={!!placesError}>
							...
							<Combobox
								...
								itemToStringLabel={(item) =>
									`${item.title ?? "Untitled"} ${item.artist_title ?? ""}`
								}
										...
										>
											<div className="flex items-start justify-between gap-2">
												<div className="min-w-0">
													...
													<p className="text-muted-foreground truncate text-xs">
														{field.artistTitle ?? "Unknown artist"} - ID{" "}
														{field.externalId}
													</p>
												</div>
												<Button
